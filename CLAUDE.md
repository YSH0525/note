...
> **이 저장소에는 앱이 두 개 있다.** 아래 내용은 전부 폴드노트(`app/`) 이야기다.
> 아기 단어카드(`cards/`)는 [아기-단어카드.md](아기-단어카드.md)를 본다 — 코드도 저장 키도
> 겹치지 않는 별개 앱이고, 점검은 `node tools/cards-check.mjs`.
> GitHub Pages 워크플로는 `app/`을 사이트 루트에, `cards/`를 `/cards/`에 함께 올린다.
...
- `build.sh`가 `cp ../app/*`로 에셋을 복사하므로 **`app/` 아래에 하위 디렉터리를 만들면 누락된다.**
  ...
  **단어카드는 `cards/img/`에 사진을 두므로 그쪽 빌드는 `cp -r ../cards/. assets/`로 해야 한다.**
  Pages 워크플로도 같은 이유로 `cp -r`을 쓴다.
